...
static void ath9k_pci_fixup(struct pci_dev *dev)
{
	...
	if (bus->resource[0])
		printk("%s: res=[%x-%x] flags=%lu\n", __func__, bus->resource[0]->start, bus->resource[0]->end, bus->resource[0]->flags);
	if (bus->resource[1])
		printk("%s: res=[%x-%x] flags=%lu\n", __func__, bus->resource[1]->start, bus->resource[1]->end, bus->resource[1]->flags);
	if (bus->resource[2])
		printk("%s: res=[%x-%x] flags=%lu\n", __func__, bus->resource[2]->start, bus->resource[2]->end, bus->resource[2]->flags);
	if (bus->resource[3])
		printk("%s: res=[%x-%x] flags=%lu\n", __func__, bus->resource[3]->start, bus->resource[3]->end, bus->resource[3]->flags);
	...
	printk("%s: start=%x mem=%px\n", __func__, priv->mem_res->start, mem);
...
#if 1
	print_hex_dump(KERN_CRIT, "ath9k", DUMP_PREFIX_OFFSET, 16, 1, priv->pdata.eeprom_data, ATH9K_PLAT_EEP_MAX_WORDS * sizeof(u16), true);
#endif
